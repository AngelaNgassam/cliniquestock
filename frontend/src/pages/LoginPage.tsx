...
function LoginForm() {
  ...
  /* ── Callback commun après obtention du token social ────────────────────── */
    ...
    <>
      ...
      <div className="cs-page">
        ...
        <div className="cs-card">
          ...
          <p style={{ fontSize:13, color:'#64748b', textAlign:'center', marginBottom:22 }}>Connectez-vous pour accéder à votre inventaire clinique.</p>
          ...
          <form onSubmit={handleSubmit}>
            <label style={{ display:'block', fontSize:12, fontWeight:600, color:'#374151', marginBottom:5 }} htmlFor="email">Adresse Email</label>
            <div style={{ position:'relative', marginBottom:14 }}>
              <span style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', color:'#94a3b8', pointerEvents:'none' }}><Mail size={14} /></span>
              <input id="email" type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="nom@clinique.fr"
                style={inputBase(focusedField === 'email')} onFocus={() => setFocusedField('email')} onBlur={() => setFocusedField(null)} />
            </div>

            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:5 }}>
              <label style={{ fontSize:12, fontWeight:600, color:'#374151' }} htmlFor="password">Mot de passe</label>
              <a href="#" style={{ fontSize:12, fontWeight:600, color:'#0ea5e9', textDecoration:'none' }}>Mot de passe oublié ?</a>
            </div>
            <div style={{ position:'relative', marginBottom:14 }}>
              ...
              <input id="password" type={showPwd ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••"
                style={inputBase(focusedField === 'password', true)} onFocus={() => setFocusedField('password')} onBlur={() => setFocusedField(null)} />
            ...
            <div style={{ marginBottom:16 }}>
              <label style={{ display:'inline-flex', alignItems:'center', gap:7, cursor:'pointer', fontSize:12, color:'#64748b' }}>
                <input type="checkbox" checked={remember} onChange={e => setRemember(e.target.checked)} style={{ accentColor:'#0ea5e9', width:14, height:14 }} />
            ...
            <button type="submit" disabled={busy} className="cs-submit"
              ...
              {loading ? <><svg style={{ width:15, height:15, animation:'spin 0.8s linear infinite' }} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="rgba(255,255,255,0.3)" strokeWidth="4"/><path fill="rgba(255,255,255,0.9)" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Connexion…</> : 'Se connecter'}
            </button>
          </form>
